...
void reportCoinsAndAddresses({
  ...
  for (final vout in utxoMap.keys) {
    ...
    print("Found $val ${type.coin.symbol} at address ${vout.getAddress(type)}");
  }
  final balance = utxoBalance / pow(10, type.coin.decimals).toBI;
  print("Total wallet balance: $balance ${type.coin.name}");
  expect(
  ...
  expect(
    (utxoBalance - visualBalance).abs(),
    lessThanOrEqualTo(BigInt.from(0.1 * 1e8)),
  );
}
